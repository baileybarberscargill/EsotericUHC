...
public class GoldenHeadsCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
        ...
        if (!sender.hasPermission("uhc.goldenheads")){
            sender.sendMessage(ChatColor.RED + "oops! Looks like you don't have permission to do that!");
            return true;
        }

        if (args.length == 0){
            sender.sendMessage(Main.PREFIX + "Usage: /goldenheads <enable|disable>");
            return true;
        }

        if (args[0].equalsIgnoreCase("enable")){
            if (enabled){
                sender.sendMessage(Main.PREFIX + ChatColor.GOLD + "Golden heads are already enabled.");
                return true;
            }

            Bukkit.broadcastMessage(Main.PREFIX + "Golden heads are now " + ChatColor.GREEN + "enabled" + ChatColor.GRAY + ".");
            enabled = true;
            goldenheads = true;
        }
        ...
        if (args[0].equalsIgnoreCase("disable")){
            if (!enabled){
                sender.sendMessage(Main.PREFIX + ChatColor.GOLD + "Golden heads are already disabled.");
                return true;
            }
            Bukkit.broadcastMessage(Main.PREFIX + "Golden heads are now " + ChatColor.RED + "disabled" + ChatColor.GRAY + ".");
            enabled = false;
            goldenheads = false;
        }
